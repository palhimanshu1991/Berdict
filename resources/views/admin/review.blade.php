...
<meta property="og:url" content="{{Config::get('url.home')}}movie/{{$film->fl_id}}/{{\Helpers\Helper::cleanUrl($film->fl_name)}}" />
<link rel='image_src' href="{{Config::get('url.home')}}uploads/movie/{{$film->fl_year}}/{{$film->fl_image}}">
<meta property='og:image' content="{{Config::get('url.home')}}uploads/movie/{{$film->fl_year}}/{{$film->fl_image}}" />
...
@if(Auth::check())
<?php $like = DB::table('review_likes')->where('review_id', $review->fr_id)->where('user_id', $random)->first(); ?>
...
<div class="container" id="mainframe">
    <div class="row-fluid" style="min-height: 50px; "> 
        <h2 style="font-weight:700;text-transform: uppercase;margin:">                    
            Short Review of 
            <a href="{{Config::get('url.home')}}movie/{{$film->fl_id}}/{{\Helpers\Helper::cleanUrl($film->fl_name)}}">"{{$film->fl_name}}"</a>                    
        </h2>
    </div>
                        ...
                        <div class="res-review-header col-md-12 pad0" style="">
                            <div class="res-review-header col-md-12 pad0" style="width:100%;height:48px;">
                                ...
                                <div class="res-review-rating col-md-6 pad0">
                                    <img class="img-responsive" src="{{Config::get('url.home')}}rate_{{$review->fr_vote}}.jpg" alt="" style="width:48px;display: inline;float:right;">
                            ...
                            <div data-review-id="31" class="res-review-body" style="font-size:16px;line-height: 24px;">
                                <div>
                                    <img style="margin-top:-5px;" src="{{Config::get('url.home')}}berdict/img/quote_b.png">  {{$review->fr_review}}     
                                </div>                                    
                            </div>
                        ...
                        <div class="col-sm-12">
                            <div class="form-group">
                                <div class="input-group pad0 col-sm-12"> 
                                    ID: 
                                    <input type="text" name="user" id="user" value="{{$random}}">
                                    <a href="{Config::get('url.home')}}{{$randomuser->username}}"> {{$randomuser->usr_fname.' '.$randomuser->usr_lname}}</a>

                                    <input type="hidden" name="review" id="review" value="{{$review->fr_id}}">
                                    <textarea name="comment" id="comment" maxlength="400" class="form-control" style="height:80px;" placeholder="commenting as id {{$random}}"></textarea>
                                </div>
                            </div>    
                        </div>  
                    </div>
