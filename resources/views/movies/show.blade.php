...
<meta property="og:url" content="{{Config::get('url.home')}}movie/{{$movie->fl_id}}/{{\Helpers\Helper::cleanUrl($movie->fl_name)}}" />
<link rel='image_src' href="{{Config::get('url.home')}}uploads/movie/{{$movie->fl_year}}/{{$movie->fl_image}}">
<meta property='og:image' content="{{Config::get('url.home')}}uploads/movie/{{$movie->fl_year}}/{{$movie->fl_image}}" />
	    ...
	    <div class="row ptop2 ">
	        <div class="col-md-9">
	            ...
	            <div class="row-fluid pad0" style="min-height:80px;margin-bottom:0px;padding-top:0px;font-size:13px;padding-bottom:10px;">
	                ...
	                <h4 style="line-height: 1.4;font-weight: 500;color: rgba(0,0,0,0.6);letter-spacing: -0.02em;">
	                    Directed by <span style="font-weight:600">{{$movie->fl_dir_ar_id}}</span> 
            ...
            @if(Auth::check())
            @if($commonFav)        
            <div class="col-md-12 pad0 hidden">
                <div class="hidden">	
                    <div class="pad0">
                        ...
                        <ul class="pad0" style="list-style: none;margin-top:15px;">
                            <li>
                                ...
                                <a href="{{Config::get('url.home')}}{{$use->username}}">
                                    <img rel="tooltip" data-placement="top" title="{{$use->usr_fname.' '.$use->usr_lname}}" data-original-title="{{$use->usr_fname.' '.$use->usr_lname}}" style="height:45px;width:45px;margin-right: 10px;border-radius:50%;  border: 1px solid #ddd;" class="lazy" src="{{Config::get('url.home')}}berdict/img/avatar_50.png" data-original="{{Config::get('url.web')}}user_uploads/1000/{{$use->id}}/{{$use->usr_image}}" alt="user photo" title="{{$use->usr_fname.' '.$use->usr_lname}}">	
                                </a>
                                @endforeach
                            </li>
                        </ul>
	        ...
	        <div class="col-md-3">
	            <div class="image">           
	                <img class="lazy img-responsive" style="  border: 1px solid rgb(204, 204, 204);max-height: 345px;min-height: 300px;" src="{{ Config::get('url.web')}}berdict/img/default_poster.jpg" data-original="{{ Config::get('url.web')}}uploads/movie/{{$movie->fl_year}}/{{$movie->fl_image}}"  />
	            </div>  
	        </div>	        
	    </div>
	</div>
...
<div>
	<div class="container ptop2">
		<div class="row">
			...
			<a href="{{Config::get('url.home')}}movie/{{$film->fl_id}}/{{\Helpers\Helper::cleanUrl($film->fl_name)}}">
		    	<div class="col-md-2 col-xs-6" style="margin-bottom:20px;">
		    		<div class="suggestion-cover ">
		    			<img width="100%" style="border:1px solid #ddd;" class="lazy img-responsive" src="{{Config::get('url.web')}}berdict/img/default_poster.jpg"  data-original="{{Config::get('url.web')}}uploads/movie/{{$film->fl_year}}/{{$film->fl_image}}">
		    		</div>
               ...
               <div class="row" style="margin:0px;">
	               <div class="res-review-user col-md-9 col-xs-9 pad0" style="height: 50px;">
	                  <a class="left" href="{{Config::get('url.home')}}{{$myReview->username}}">
	                    <img class="lazy img-responsive " src="{{Config::get('url.home')}}berdict/img/avatar_50.png" data-original="{{Config::get('url.web')}}user_uploads/1000/{{$myReview->id}}/{{$myReview->usr_image}}" alt="" style="height:36px;width: 36px; display: inline;border-radius:50px;">
	                  </a>
	               ...
	               <div class="res-review-rating col-md-3 col-xs-3 pad0" style="">
                   	<img class="img-responsive" src="{{Config::get('url.home')}}rate_{{$film->fr_vote}}.jpg" alt="" style="width:36px;display: inline;float:right;">
               ...
               <div class="res-review-header col-md-12 pad0" style="height:30px;">
                  <div class="res-review-user col-md-12 col-xs-9 pad0" style="">
                     <div data-review-id="4477" class="res-review-body review-profile feed-review-body" style="font-weight: 800;font-size: 22px;margin-bottom:0;">
                        <a class="review-headline" href="{{Config::get('url.home')}}movie/{{$film->fl_id}}/{{\Helpers\Helper::cleanUrl($film->fl_name)}}"> {{$film->fr_review}} </a>
                     </div>
               ...
               <div review-container="{{$film->fr_id}}" class="comment-container-{{$film->fr_id}} hidden comment-wrapper">
                  ...
                  <div class="res-review-user col-md-12 pad0" style="padding:10px 0px;border-top:1px solid #eee;">
                     <a class="left" href="{{Config::get('url.home')}}{{$reply->username}}">
                        <img class="lazy img-responsive " src="{{Config::get('url.home')}}berdict/img/avatar_50.png" data-original="{{Config::get('url.web')}}user_uploads/1000/{{$reply->id}}/{{$reply->usr_image}}" alt="" style="height:36px;width: 36px; display: inline;border-radius:50px;">
                     </a>
                     <div class="feed-rate-user-details">
               ...
               <div class="row" style="margin:0px;">
	               <div class="res-review-user col-md-9 col-xs-9 pad0" style="height: 50px;">
	                  <a class="left" href="{{Config::get('url.home')}}{{$review->username}}">
	                    <img class="lazy img-responsive " src="{{Config::get('url.home')}}berdict/img/avatar_50.png" data-original="{{Config::get('url.web')}}user_uploads/1000/{{$review->id}}/{{$review->usr_image}}" alt="" style="height:36px;width: 36px; display: inline;border-radius:50px;">
	                  </a>
	                  <div class="feed-rate-user-details">
...
<script type="text/javascript">
	...
	// 				$.ajax({
	// 					type: "POST",
	// 					url: HOST + "movie/{{$movie->fl_id}}/omdbUpdate",
	// 			        dataType: 'json',
	// 			        data: {info: response},
	// 					cache: true,
	// 					success: function(blah)
	// 					{
	// 					    console.log('omdbUpdated');
	// 					}
	// 				});			    
	// 		    }
	// 		}
